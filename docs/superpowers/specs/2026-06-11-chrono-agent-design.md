...
- **暂不吸收**：外部数据导入 API、插件市场、复杂任务同步、知识图谱、完整向量检索平台、第三方应用生态、独立待办系统、复杂付费额度系统。
...
检索索引不是独立记忆层，只是实现细节。MVP 先使用 PostgreSQL 中的时间、类型、人物、标签和关键词过滤；embedding 引用保留为可选字段，后续可以替换为向量索引。Agent 回复必须优先基于带证据的会话记录和个人记忆，不能只基于索引摘要得出高风险结论。
...
- Memory 召回失败时仍允许生成保守回复，但需要在 `agent_run` 中记录降级原因。
...
- Java 后端使用 Maven 还是 Gradle。
- 持久化层使用 JPA 还是 MyBatis。
- 本地开发是否使用 Docker Compose 启动 PostgreSQL。
- 假模型跑通后，真实的转写、说话人分离、声纹向量和 LLM 服务选型。
- 流式音频第一期优先使用 WebSocket、HTTP 分片，还是两者都支持。
- 低价值会话过滤阈值，例如最短时长、最少有效语音秒数、最低转写置信度。
- 会话后处理失败的最大重试次数和退避策略。
- 个人记忆检索第一期使用 PostgreSQL 过滤；是否引入向量索引放到后续阶段评估。
- 哪些个人记忆类型允许自动保存，哪些必须用户确认。
- 原始音频和声纹向量的具体保留周期。
- 会话消息和个人记忆的默认保留周期。
